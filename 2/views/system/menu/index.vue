...
export default {
  ...
  computed: {
    ...
    calculatedTableHeight() {
      // 搜索表单：约 60px（展开时）或 0（收起时）
      // 工具栏：约 40px
      // 页面 padding：40px（上下各20px）
      // 其他边距：约 20px
      const searchHeight = this.showSearch ? 60 : 0;
      const toolbarHeight = 40;
      const padding = 40;
      const margins = 20;
      const reserved = searchHeight + toolbarHeight + padding + margins;
      
      // 视口高度 - 保留空间，最小 300px
      const viewportHeight = window.innerHeight || 800;
      const calculated = viewportHeight - reserved - 94; // 94px 是顶部导航栏高度
