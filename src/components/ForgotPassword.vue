<template>
  <div class="forgot-password-wrapper">
    ...
    <h2 class="forgot-title">{{ $t('auth.forgotPassword.title') }}</h2>
    <p class="forgot-hint">
      {{ $t('auth.forgotPassword.hint1') }}<br />
      {{ $t('auth.forgotPassword.hint2') }}
    </p>
    ...
    >
        ...
        >
          <template #suffix>
            <el-button
              type="primary"
              link
              :disabled="!isAccountValidForForgot || forgotCodeSending || forgotCountdown > 0"
              @click="sendForgotCode"
              tabindex="-1"
              @mousedown.prevent
            >
              {{ forgotCountdown > 0 ? $t('auth.login.codeSentCountdown', { seconds: forgotCountdown }) : $t('auth.register.getCode') }}
            </el-button>
          </template>
        </el-input>
      </el-form-item>
      ...
      <el-form-item>
        <el-button size="large" class="forgot-btn" @click="$emit('close')">
...
// 判断账号是否合法（手机号或邮箱格式）
const isAccountValidForForgot = computed(() => {
  const account = forgotForm.account;
